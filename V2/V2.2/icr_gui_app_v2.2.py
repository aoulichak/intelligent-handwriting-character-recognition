"""
Application GUI de Reconnaissance de Caractères Manuscrits (ICR) - Version 2.2
Utilise PyQt5 pour l'interface graphique et un modèle CNN PyTorch pour la prédiction
Modèle: ICR V2.2 (fine-tuned sur A-Z Handwritten + EMNIST)
"""
...
# ============================================================================
# ARCHITECTURE DU MODÈLE CNN V2.2 (identique à l'entraînement)
# ============================================================================
...
class DrawingCanvas(QLabel):
    ...
    def get_image(self):
        """
        Retourne l'image du canvas comme array NumPy prétraité pour le modèle V2.2.
        Utilise la normalisation spécifique au modèle V2.2: Mean=0.7465, Std=0.2650
        """
        ...
        # Normaliser avec les paramètres du modèle V2.2
        # Mean=0.7465, Std=0.2650
        img_array = (img_array - 0.7465) / 0.2650
...
class ICRMainWindow(QMainWindow):
    """
    Fenêtre principale de l'application ICR V2.2.
    """
    def __init__(self):
        super().__init__()
        
        self.setWindowTitle("ICR - V2.2 - Reconnaissance de Caractères Manuscrits")
    ...
    def load_model(self, model_path=None):
        """Charge le modèle CNN V2.2."""
        if model_path is None:
            # Chercher le modèle dans le répertoire courant
            possible_paths = [
                'icr_v2.2_best.pth',
                'icr_v2_2_best.pth',
                os.path.join(os.path.dirname(__file__), 'icr_v2.2_best.pth'),
                os.path.join(os.path.dirname(__file__), 'icr_v2_2_best.pth'),
            ]
        ...
        if model_path and os.path.exists(model_path):
            try:
                self.model = ICR_V2_Model(num_classes=26, dropout_rate=0.5).to(self.device)
                ...
                self.model_status_label.setText(f"✅ Modèle V2.2 chargé: {os.path.basename(model_path)}")
                self.model_status_label.setStyleSheet("color: #27ae60;")
                self.statusBar().showMessage(f"Modèle V2.2 chargé avec succès depuis {model_path}", 3000)
    ...
    def predict_drawing(self):
        ...
        try:
            # Obtenir l'image prétraitée (avec normalisation V2.2)
            img_array = self.canvas.get_image()
            
            # Afficher l'aperçu (dénormalisé pour l'affichage)
    ...
    def show_preview(self, img_array):
        ...
        img_denorm = (img_array * 0.2650) + 0.7465
        img_denorm = np.clip(img_denorm, 0, 1)
        
        # Convertir en 0-255
        img_display = (img_denorm * 255).astype(np.uint8)
    ...
    def show_about(self):
        ...
        QMessageBox.about(
            self,
            "À propos de ICR V2.2",
            """<h2>ICR - Reconnaissance de Caractères Manuscrits</h2>
            <p><b>Version:</b> 2.2</p>
            <p>Application de reconnaissance de caractères manuscrits (A-Z) 
            utilisant un modèle CNN amélioré entraîné sur les datasets:</p>
            <ul>
                <li>TRDG (Text Recognition Data Generator) - Données synthétiques</li>
                <li>A-Z Handwritten Alphabets (Kaggle)</li>
                <li>EMNIST Letters</li>
            </ul>
            <p><b>Améliorations V2.2:</b></p>
            <ul>
                <li>Fine-tuning sur données réelles (A-Z + EMNIST)</li>
                <li>Architecture CNN avec Batch Normalization</li>
                <li>Normalisation optimisée (Mean=0.7465, Std=0.2650)</li>
                <li>Meilleure généralisation sur écritures variées</li>
            </ul>
